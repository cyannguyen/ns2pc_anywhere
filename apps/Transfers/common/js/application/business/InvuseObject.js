...
/**@memberOf application.business.InvuseObject */
		...
		shipped: function(invuse, statusDate, memo, eventContext){				
			var comp = InvuseStatusHandler.getInstance().toDefaultExternalLabel("SHIPPED");
			this.changeStatus(invuse, comp, statusDate, memo, eventContext);
		},
		...
		changeStatus: function(invuse, newStatus, statusDate, memo, eventContext){
			...
			if(InvuseStatusHandler.getInstance().canPerformTransition(currentStatus, newStatus)){
				...
				if (newStatus=='SHIPPED'){
					...
					invuse.set('shipdate',tempShipmentResource.shipdate);
					invuse.set('expreceiptdate',tempShipmentResource.expreceiptdate);
					invuse.set('carrier',tempShipmentResource.carrier);
					invuse.set('packingslipnum',tempShipmentResource.packingslipnum);
					invuse.set('shiptoattn',tempShipmentResource.shiptoattn);
					invuse.set('shipto',tempShipmentResource.shipto);
